...
// All dependent modules' source code should be preloaded in jsdata.go, or loaded before this module.
...
// It first check if the module is already in cache, then check if it is a preloaded resource in jsdata.go.
...
func (m *Engine) registerResources() error {
	...
	// add all other known asset from jsdata.go
	assets := AssetNames()
...
// assetName is the asset ID in jsdata.go, it may not be the same as the module name in request('module')
